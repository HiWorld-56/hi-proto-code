...
;
                        ...
                        GeneratedField::UsdsFuturesTicker24h => {
                            if op__.is_some() {
...
impl serde::Serialize for BinanceFuturesTicker24h {
    #[allow(deprecated)]
    fn serialize<S>(&self, serializer: S) -> std::result::Result<S::Ok, S::Error>
    where
        S: serde::Serializer,
    {
        ...
        let mut struct_ser = serializer.serialize_struct("hi.binance.BinanceFuturesTicker24h", len)?;
        if let Some(v) = self.symbol.as_ref() {
            struct_ser.serialize_field("symbol", v)?;
        }
        struct_ser.end()
...
impl<'de> serde::Deserialize<'de> for BinanceFuturesTicker24h {
    #[allow(deprecated)]
    fn deserialize<D>(deserializer: D) -> std::result::Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        ...
        struct GeneratedVisitor;
        impl<'de> serde::de::Visitor<'de> for GeneratedVisitor {
            type Value = BinanceFuturesTicker24h;

            fn expecting(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
                formatter.write_str("struct hi.binance.BinanceFuturesTicker24h")
            }

            fn visit_map<V>(self, mut map_: V) -> std::result::Result<BinanceFuturesTicker24h, V::Error>
                where
                    V: serde::de::MapAccess<'de>,
            {
                let mut symbol__ = None;
