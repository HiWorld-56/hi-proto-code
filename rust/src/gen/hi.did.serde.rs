...
impl serde::Serialize for UploadUserAvatarReq {
    #[allow(deprecated)]
    fn serialize<S>(&self, serializer: S) -> std::result::Result<S::Ok, S::Error>
    where
        S: serde::Serializer,
    {
        ...
        let mut struct_ser = serializer.serialize_struct("hi.did.UploadUserAvatarReq", len)?;
        if !self.user.is_empty() {
            struct_ser.serialize_field("user", &self.user)?;
        }
        if !self.name.is_empty() {
...
impl<'de> serde::Deserialize<'de> for UploadUserAvatarReq {
    #[allow(deprecated)]
    fn deserialize<D>(deserializer: D) -> std::result::Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        ...
        struct GeneratedVisitor;
        impl<'de> serde::de::Visitor<'de> for GeneratedVisitor {
            type Value = UploadUserAvatarReq;

            fn expecting(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
                formatter.write_str("struct hi.did.UploadUserAvatarReq")
            }

            fn visit_map<V>(self, mut map_: V) -> std::result::Result<UploadUserAvatarReq, V::Error>
                where
                    V: serde::de::MapAccess<'de>,
            {
                let mut user__ = None;
        ...
        deserializer.deserialize_struct("hi.did.UploadUserAvatarReq", FIELDS, GeneratedVisitor)
    }
}
