...
impl serde::Serialize for ListMyAgentsResp {
    #[allow(deprecated)]
    fn serialize<S>(&self, serializer: S) -> std::result::Result<S::Ok, S::Error>
    where
        S: serde::Serializer,
    {
        ...
        let mut struct_ser = serializer.serialize_struct("hi.club.ListMyAgentsResp", len)?;
        if self.total != 0 {
            struct_ser.serialize_field("total", &self.total)?;
        }
        if !self.agents.is_empty() {
...
impl<'de> serde::Deserialize<'de> for ListMyAgentsResp {
    #[allow(deprecated)]
    fn deserialize<D>(deserializer: D) -> std::result::Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        ...
        struct GeneratedVisitor;
        impl<'de> serde::de::Visitor<'de> for GeneratedVisitor {
            type Value = ListMyAgentsResp;

            fn expecting(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
                formatter.write_str("struct hi.club.ListMyAgentsResp")
            }

            fn visit_map<V>(self, mut map_: V) -> std::result::Result<ListMyAgentsResp, V::Error>
                where
                    V: serde::de::MapAccess<'de>,
            {
                let mut total__ = None;
        ...
        deserializer.deserialize_struct("hi.club.ListMyAgentsResp", FIELDS, GeneratedVisitor)
    }
}
